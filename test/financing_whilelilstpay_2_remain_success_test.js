...
contract("FinancingTest-whilepay-2-remain-success", (accounts) => {

    let user = accounts[5]
    let nftUser = accounts[4]
    ...
    //NFT pause
    it("testing transfer-test() should assert true", async function () {
        //paused()
        const financing = await Financing.deployed()
        const receiptNFT = await financing.receiptNFT()
        const nftImpl = await NFTImpl.at(receiptNFT)
        const status = await nftImpl.paused()
        console.log(`nft status ${status}`)


        const nftBalance = await nft.balanceOf(receiptNFT, user)
        console.log(`nft balance ${nftBalance}`)
        const owner = await nft.ownerOf(receiptNFT, 17)
        expect(owner).to.equal(user)
        try {
            await nftImpl.transferFrom(user, nftUser, 17, { from: user })
          } catch (error) {
            console.log(`transfer-test( ${error} )`)
