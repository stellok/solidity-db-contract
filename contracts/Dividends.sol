...
contract Dividends is ReentrancyGuard {
    ...
    function doMonthlyTask(uint256 index) public nonReentrant {
        ...
        require(monthlyList[index] + expire < block.timestamp, "no time"); // 没到时间呢
        require(
            receiveRecord[monthlyList[index]].exist == false,
            "has been comforted"
        ); // 已经被舒适化
    ...
    function receiveDividends(
        ...
        require(
            receiveRecord[monthlyList[index]].totalMonthlyBalance >=
                tokenList.length * receiveRecord[monthlyList[index]].dividend,
        ...
        for (uint i = 0; i < tokenList.length; i++) {
            ...
            require(
                receiveRecord[monthlyList[index]].isReceive[tokenList[i]] ==
                    false,
                "Cannot be claimed repeatedly"
            ); // 无法重复领取

            receiveRecord[monthlyList[index]].isReceive[tokenList[i]] == true;
            receiveRecord[monthlyList[index]]
                .totalMonthlyBalance -= receiveRecord[monthlyList[index]]
                .dividend;
        }
        USDT.safeTransfer(
            msg.sender,
